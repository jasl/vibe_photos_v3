...
"""
    ...
    with open("poc1/requirements.txt", "w") as f:
...
def create_config_file():
    ...
    with open("poc1/config.json", "w") as f:
...
app = FastAPI(title="Vibe Photos PoC1")
...
@app.get("/")
def read_root():
    return {"message": "Vibe Photos PoC1 API", "status": "running"}
...
'''
    ...
    # 保存脚本
    with open("poc1/scripts/init_db.py", "w") as f:
        f.write(init_db_script)
    
    with open("poc1/app/main.py", "w") as f:
        f.write(main_app_script)
        
    with open("poc1/ui/app.py", "w") as f:
        f.write(ui_script)
    
    # 创建__init__.py文件
    for path in ["poc1/app/__init__.py", "poc1/processors/__init__.py"]:
        Path(path).touch()
...
"""
    ...
    with open("poc1/README.md", "w") as f:
        f.write(readme)
