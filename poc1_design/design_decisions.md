...
| 方面 | V3原设计 | PoC1简化版 | 简化原因 |
|------|----------|------------|----------|
| 处理模式 | 实时+异步 | 纯离线批处理 | 降低复杂度 |
| 识别层次 | 大类→子类→型号 | 仅大类 | 验证核心功能 |
...
```mermaid
graph LR
    A[PoC1<br/>离线批处理] --> B{效果评估}
    B -->|识别质量OK| C[PoC2<br/>优化搜索]
    B -->|识别质量差| D[调整技术方案]
    C --> E[MVP<br/>实时处理]
    D --> F[重新选型]
    E --> G[V1.0<br/>完整功能]
...
### 决策4：使用RTMDet作为主要识别引擎
- **时间：** 2024-11-XX
- **原因：** 
  - 更高的检测精度（52.8% mAP vs CLIP的~40%）
  - 专为物体检测设计，输出精确边界框
  - Apache-2.0许可，商用友好
  - 更适合验证识别质量的目标
- **影响：** 需要安装MMDetection，配置稍复杂
- **替代方案：** 保留CLIP作为备选，可随时切换

## ✅ 总结

PoC1的设计充分考虑了审查者的反馈，特别是关于系统复杂度的担忧。我们采取了**极简主义**的方法：
