...
| ✅ | 🔴 P0 | DET-001 | Implement SigLIP loader abstraction. | ENV-004 | `src/models/siglip.py` | Added lazy Hugging Face pipeline wrapper. |
| ✅ | 🔴 P0 | DET-002 | Implement BLIP loader abstraction. | ENV-004 | `src/models/blip.py` | BLIP captioner integrated with detector. |
| ✅ | 🔴 P0 | DET-003 | Compose unified detector (labels + captions). | DET-001, DET-002 | `src/core/detector.py` | Unified SigLIP+BLIP detector implemented. |
| ✅ | 🟠 P1 | DET-004 | Batch processor orchestrating detector + OCR. | DET-003, OCR-001 | `src/core/processor.py` | Batch processor with dedupe + stats. |
| ✅ | 🔴 P0 | OCR-001 | PaddleOCR service with caching + batching. | ENV-004 | `src/core/ocr.py` | PaddleOCR wrapper with lazy init. |
| ✅ | 🔴 P0 | DB-001 | Define SQLite schema & migrations. | ENV-001 | `src/core/database.py`, migrations | ORM models + init helpers created. |
| ✅ | 🔴 P0 | DB-002 | Persistence services (CRUD + search helpers). | DB-001 | Repository classes/tests | Asset repository + search helpers delivered. |
| ✅ | 🔴 P0 | API-001 | FastAPI app factory + health endpoint. | DB-002 | `src/api/main.py` | App factory + health route wired. |
