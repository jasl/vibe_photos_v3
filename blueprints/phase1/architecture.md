# Phase 1 技术架构设计

## 🎯 设计原则

1. **极简主义** - 每个组件都是必需的，没有冗余
2. **单体优先** - 避免分布式复杂性
3. **批处理思维** - 离线处理，在线查询
4. **渐进增强** - 可以轻松升级但不过度设计

## 🏗 系统架构

### 整体架构图

```
┌─────────────────────────────────────────────────────┐
│                    用户界面层                        │
│                                                      │
│  ┌──────────────┐        ┌──────────────────────┐   │
│  │  Web UI      │        │  CLI Tool            │   │
│  │ (Streamlit-  │        │ (process_dataset.py  │   │
│  │  浏览/搜索)  │        │  批处理脚本)        │   │
│  └──────┬───────┘        └──────────┬───────────┘   │
│         │                           │                │
└─────────┼───────────────────────────┼────────────────┘
          │                           │
┌─────────▼───────────────────────────▼────────────────┐
│                    API服务层                          │
│                                                       │
│  ┌─────────────────────────────────────────────┐    │
│  │           FastAPI Application               │    │
│  │  ┌─────────┐  ┌──────────┐  ┌───────────┐  │    │
│  │  │ 搜索API │   │ 浏览API   │  │ 批处理API |  │    │
│  │  └─────────┘  └──────────┘  └───────────┘  │    │
│  │  ┌─────────┐  ┌──────────┐                  │    │
│  │  │ 搜索API │  │ 浏览API  │                  │    │
│  │  └─────────┘  └──────────┘                  │    │
│  └─────────────────────────────────────────────┘    │
│                                                       │
└───────────────────────────────────────────────────────┘
                            │
┌───────────────────────────▼───────────────────────────┐
│                    处理引擎层                          │
│                                                       │
│  ┌──────────────┐  ┌──────────────┐  ┌────────────┐   │
│  │ 图像识别引擎   |  | OCR引擎       │  | 批处理器   │     │
│  │(SigLIP+BLIP) │  │ (PaddleOCR)  │  │ (CLI驱动)  │    │
│  └──────────────┘  └──────────────┘  └────────────┘    │
│                                                        │
└────────────────────────────────────────────────────────┘
                            │
┌───────────────────────────▼───────────────────────────┐
│                    数据存储层                          │
│                                                        │
│  ┌──────────────────────────────────────────────┐    │
│  │              SQLite数据库                     │    │
│  │  ┌──────────┐  ┌──────────┐  ┌───────────┐  │    │
│  │  │ 图片元数据│  │ 识别结果   │  | OCR文本    |  │    │
│  │  └──────────┘  └──────────┘  └───────────┘  │    │
│  └──────────────────────────────────────────────┘    │
│                                                        │
│  ┌──────────────────────────────────────────────┐    │
│  │              本地文件系统                     │    │
│  │    原始图片 / 缩略图 / 处理缓存              │    │
│  └──────────────────────────────────────────────┘    │
│                                                        │
└────────────────────────────────────────────────────────┘
```

- **Web UI (Streamlit)**：提供搜索与处理状态视图，仅通过FastAPI读取数据，不负责导入。
- **CLI Tool (`process_dataset.py`)**：执行所有批处理导入任务，直接调用处理引擎并写入数据库/缓存，不通过FastAPI暴露的接口。

## 📦 技术栈选择

### 核心依赖（必需）
```python
# requirements.txt - 2024年11月最新稳定版本
fastapi==0.121.1           # API框架 (最新版本)
uvicorn==0.38.0            # ASGI服务器 (最新版本)
streamlit==1.51.0          # 快速Web UI
sqlalchemy==2.0.44         # ORM
sqlite3                    # 内置数据库
pillow==11.3.0             # 图像处理
python-multipart==0.0.20   # 文件上传支持
aiofiles==24.1.0           # 异步文件操作
pydantic==2.11.10          # 数据验证
```

### 运行环境假设

- **开发环境**：macOS，Apple M4 Max（充足CPU/GPU核与内存），本地执行批处理与功能开发。
- **测试环境**：高端PC，NVIDIA GeForce 5090 GPU（用于验证SigLIP+BLIP 和 PaddleOCR推理表现）。
- **存储前提**：由维护者提供足够的本地磁盘容量，无需规划云端资源。
- **部署目标**：单机离线部署，不涉及云端或分布式部署方案。

### 识别引擎
```python
# 主方案：SigLIP+BLIP (多语言支持)
torch==2.9.1                  # PyTorch (最新版本)
torchvision==0.24.1           # TorchVision (最新版本)
transformers==4.57.1          # Hugging Face Transformers（用于SigLIP和BLIP）
sentence-transformers==5.1.2  # 句子嵌入（用于语义搜索）

# 备选方案：SigLIP (更强大，可选安装)
# transformers==4.57.1    # Hugging Face
# 模型: google/siglip-base-patch16-224-i18n
```

### OCR引擎
```python
paddlepaddle==3.2.1       # PaddlePaddle框架最新稳定版
paddleocr==3.3.1          # PaddleOCR最新稳定版
```

## 🗄 数据库设计

### 简化的表结构（支持未来扩展）
```sql
-- 图片主表
CREATE TABLE images (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    filename TEXT NOT NULL,
    original_path TEXT NOT NULL,      -- 原始图片路径
    processed_path TEXT,               -- 归一化后的图片路径
    thumbnail_path TEXT,               -- 缩略图路径
    phash TEXT UNIQUE,                 -- 感知哈希（用于去重）
    file_size INTEGER,
    width INTEGER,
    height INTEGER,
    format TEXT,
    import_time DATETIME DEFAULT CURRENT_TIMESTAMP,
    process_status TEXT DEFAULT 'pending',  -- pending/processing/completed/failed/duplicate
    process_time FLOAT,
    error_message TEXT,                -- 错误信息（如果失败）
    
    -- 🔄 预留字段（Phase 2准备）
    embedding_json TEXT,  -- 存储向量嵌入（JSON格式）
    caption TEXT         -- 存储图像描述（未来扩展）
);

-- 创建索引
CREATE INDEX idx_phash ON images(phash);  -- 快速去重查询
CREATE INDEX idx_status ON images(process_status);  -- 状态查询
CREATE INDEX idx_import_time ON images(import_time);  -- 时间排序

-- 检测结果表
CREATE TABLE detections (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    image_id INTEGER NOT NULL,
    object_class TEXT NOT NULL,      -- 检测到的物体类别
    confidence FLOAT NOT NULL,
    bbox_x1 FLOAT,
    bbox_y1 FLOAT,
    bbox_x2 FLOAT,
    bbox_y2 FLOAT,
    FOREIGN KEY (image_id) REFERENCES images(id)
);

-- OCR结果表
CREATE TABLE ocr_results (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    image_id INTEGER NOT NULL,
    text_content TEXT,
    confidence FLOAT,
    language TEXT,
    FOREIGN KEY (image_id) REFERENCES images(id)
);

-- 搜索索引表（支持全文搜索）
CREATE VIRTUAL TABLE search_index USING fts5(
    image_id,
    content,  -- 合并的可搜索文本
    tokenize = 'porter unicode61'  -- 更好的分词支持
);

-- 🔄 向量索引表（Phase 2准备，暂不激活）
-- CREATE TABLE vector_index (
--     id INTEGER PRIMARY KEY,
--     image_id INTEGER NOT NULL,
--     embedding BLOB,  -- 二进制格式存储向量
--     FOREIGN KEY (image_id) REFERENCES images(id)
-- );
```

## 🔄 处理流程

### 批处理流程（带缓存优化）
```python
async def batch_process_images(dataset_dir: str = 'samples'):
    """
    离线批处理主流程（只读源数据，缓存结果）
    """
    # 1. 扫描只读数据集
    images = scan_folder(dataset_dir, readonly=True)
    for image_path in images:
        db.add_image(image_path, status='pending')
    
    # 2. 批量处理（使用缓存）
    batch_size = 10
    for batch in chunks(images, batch_size):
        for image_path in batch:
            # 计算感知哈希（缓存键）
            phash = compute_phash(image_path)
            
            # 2.1 生成缩略图（cache/images/thumbnails/）
            create_or_load_thumbnail(image_path, phash)
            
            # 2.2 物体检测（cache/detections/）
            detections = get_or_compute_detections(image_path, phash)
            db.save_detections(detections)
            
            # 2.3 OCR提取（cache/ocr/）
            if has_text_content(image_path):
                ocr_results = get_or_compute_ocr(image_path, phash)
                db.save_ocr_results(ocr_results)
            
            # 2.4 更新搜索索引（data/vibe_photos.db）
            update_search_index(image_path)
            
            # 2.5 标记完成
            db.update_status(image_path, 'completed')
```

### 搜索流程
```python
def search_images(query: str, filters: dict = None):
    """
    快速检索流程
    """
    # 1. 全文搜索
    text_matches = db.search_fulltext(query)
    
    # 2. 类别过滤（如果指定）
    if filters.get('category'):
        text_matches = filter_by_category(text_matches, filters['category'])
    
    # 3. 置信度过滤
    if filters.get('min_confidence'):
        text_matches = filter_by_confidence(text_matches, filters['min_confidence'])
    
    # 4. 返回结果
    return {
        'results': text_matches,
        'total': len(text_matches),
        'query_time': measure_time()
    }
```

## 🚀 部署方案

### 开发环境
```bash
# 1. 创建并激活虚拟环境（使用 uv）
uv venv
source .venv/bin/activate  # Windows: .venv\Scripts\activate

# 2. 同步依赖（使用 uv）
uv sync

# 3. 初始化数据库
python scripts/init_db.py

# 4. 启动API服务
uvicorn app.main:app --reload

# 5. 启动Web UI（新终端）
streamlit run ui/app.py
```

### 目录结构
```
phase1/
├── app/
│   ├── __init__.py
│   ├── main.py              # FastAPI主应用
│   ├── models.py            # 数据模型
│   ├── database.py          # 数据库连接
│   └── api/
│       ├── search.py        # 搜索API
│       └── browse.py        # 浏览API
├── processors/
│   ├── __init__.py
│   ├── detector.py          # 物体检测
│   ├── ocr.py              # OCR处理
│   └── batch.py            # 批处理器
├── ui/
│   ├── app.py              # Streamlit应用
│   └── components/         # UI组件
├── scripts/
│   ├── init_db.py          # 数据库初始化
│   └── process_dataset.py  # CLI批处理脚本
├── tests/
│   └── test_*.py           # 测试文件
├── data/
│   ├── images/             # 原始图片
│   ├── thumbnails/         # 缩略图
│   └── cache/             # 处理缓存
├── requirements.txt
└── README.md
```

## 🎯 性能目标（非强制）

- **批处理速度**: 10-20张/分钟（CPU）
- **搜索响应**: < 1秒（1万张图片）
- **内存占用**: < 2GB
- **存储占用**: 原图 + 20%（缩略图和元数据）

## ⚙️ 配置示例

```python
# config.py
class Settings:
    # 数据库
    DATABASE_URL = "sqlite:///./phase1.db"
    
    # 批处理
    BATCH_SIZE = 100  # 适配大文件，每批100张
    MAX_WORKERS = 8   # 充分利用多核CPU
    CHECKPOINT_INTERVAL = 500  # 每500张保存进度
    
    # 图像处理
    THUMBNAIL_SIZE = (512, 512)  # 增加缩略图质量
    THUMBNAIL_QUALITY = 85  # 压缩质量
    MAX_IMAGE_SIZE = 60 * 1024 * 1024  # 60MB上限
    SUPPORTED_FORMATS = ['.jpg', '.jpeg', '.png', '.webp', '.heic']
    USE_WEBP = True  # 缩略图使用WebP格式节省空间
    
    # 检测引擎
    DETECTION_MODEL = "siglip-base"  # SigLIP (~85%准确率, 多语言支持)
    # DETECTION_MODEL = "siglip-base"  # 备选方案（多语言支持）
    CONFIDENCE_THRESHOLD = 0.3
    
    # OCR
    OCR_LANGS = ['ch', 'en']
    
    # 搜索
    SEARCH_LIMIT = 50
    
    # 存储路径（分离源数据、缓存和数据库）
    DATASET_PATH = "samples"  # 只读源数据
    THUMBNAIL_PATH = "cache/images/thumbnails"  # 可复用缓存
    PROCESSED_PATH = "cache/images/processed"  # 归一化图片
    DETECTION_CACHE = "cache/detections"  # 检测结果缓存
    OCR_CACHE = "cache/ocr"  # OCR结果缓存
    DATABASE_PATH = "data/vibe_photos.db"  # 数据库
```

## 📝 注意事项

1. **不追求实时性** - 批处理优先，用户可以等待
2. **不过度优化** - 代码清晰比性能重要
3. **不处理边缘情况** - 专注核心功能
4. **不考虑扩展性** - 单机单用户即可
5. **保持简单** - 如果一个功能需要复杂实现，考虑砍掉

## 下一步

→ 查看[实施计划](implementation.md)了解具体开发步骤
