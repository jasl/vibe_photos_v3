...
Phase 1阶段**不支持图片上传**，所有测试使用本地数据集目录。系统支持增量处理，可以随时添加新图片到数据集中。非图片文件（如MOV、MP4、RAW等）会在扫描时自动跳过，无需手动清理。
...
dataset:
  ...
  supported_formats:
    ...
    - .heic
    - .webp
  ignore_others: true       # 跳过任何未列出的扩展名（例如 .mov、.mp4、.raw）
```
