...
1. **源代码全英文**：所有.py文件的代码、注释、文档字符串必须使用英文
2. **严格遵循Python 3.12和uv**：不要使用pip或其他包管理器
3. **函数式编程优先**：避免不必要的类和继承
4. **早期返回模式**：处理错误和边界情况要尽早返回
5. **异步优先**：使用async/await处理I/O操作
6. **类型注解必须**：所有函数签名必须有类型提示
7. **Pydantic验证**：使用Pydantic模型进行输入验证
8. **错误处理完整**：每个可能失败的操作都要有错误处理
9. **日志记录充分**：关键操作必须记录日志（英文）
10. **测试驱动开发**：先写测试，再写实现
11. **文档即代码**：代码注释要清晰完整（英文）

### 代码质量要求

```python
# ✅ Good example - All English
...
# ❌ Bad example - Missing type hints and error handling
